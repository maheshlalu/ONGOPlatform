...
- (void)setupColumnNames {
	...
	for (columnIdx = 0; columnIdx < _fieldCount; columnIdx++) {
		columnNameToIndexMap[@(sqlite3_column_name(dbps, columnIdx))] = @(columnIdx);
	}
...
-(NSArray*)allFields
{
    __block NSMutableArray* fieldsList = [NSMutableArray new];
	...
	dispatch_sync(dbAccessQueue, ^{
        ...
        for(int index = 0; index < _fieldCount; ++index)
        {
            ...
            ABSQLiteField *field = [[ABSQLiteField alloc] initWithSQLitePreparedStatement:dbps withName:fieldName dispatchQueue:dbAccessQueue withfieldIndex:index];
            [fieldsList addObject:field];
        }
